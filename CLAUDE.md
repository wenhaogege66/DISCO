...
conda run -n DDBC python DISCO/datasets/MovieLens-20M/len60/convert_disco_to_tiger_letter.py
```
...
## 16. Full Data Pipeline（新数据集加入流程）

以 MovieLens-20M 为示例。加入新数据集时按以下步骤操作：

### Step 1: TIGER 处理原始数据

```bash
# 将原始数据文件放入 TIGER cache 目录
# ML-20M: ratings.csv + movies.csv → TIGER/cache/MovieLens-20M/raw/

# 运行 TIGER 数据处理（生成 embedding + semantic ID）
bash TIGER/train_tiger_movielens20m.sh 60
```

TIGER 会生成以下关键文件到 `TIGER/cache/<Dataset>/len<N>/processed/`：
| 文件 | 用途 |
|------|------|
| `sentence-t5-base.sent_emb` | Item embedding（64维）→ DISCO 的 `clhe.pt` |
| `sentence-t5-base_256,256,256,256.sem_ids` | Semantic ID（3 codebooks × 256）→ DISCO 的 `clhe_sid.npy` / `clhe_token.json` |
| `id_mapping.json` | Item ID 映射（raw ↔ 0-based），所有模型共享 |
| `all_item_seqs.json` | 原始用户序列（raw item IDs） |
| `metadata.sentence.json` | Item 文本描述 |

### Step 2: DISCO 数据派生（包含 Semantic ID）

```bash
python DISCO/convert_movielens_to_ddbc.py --seq_len 60
```

此脚本做两件事：
1. **DISCO 数据** — 读取 TIGER 的 `all_item_seqs.json` + `id_mapping.json`，映射为 0-based IDs，按 70/10/20 切分 train/valid/test，写入 `DISCO/datasets/MovieLens-20M/len60/train.txt` 等
2. **语义文件** — 将 TIGER 的 semantic ID 转为 DISCO 格式（`clhe_sid.npy`, `clhe_token.json`, `clhe_weight.npy`），同时导出 LETTER 的 `.index.json`

### Step 3: 派生到所有 Baseline

```bash
# 从 DISCO train.txt 派生所有 baseline 训练数据 & TIGER/LETTER 训练数据
conda run -n DDBC python DISCO/datasets/MovieLens-20M/len60/convert_disco_to_dreamrec.py
conda run -n DDBC python DISCO/datasets/MovieLens-20M/len60/convert_disco_to_diffurec.py
conda run -n DDBC python DISCO/datasets/MovieLens-20M/len60/convert_disco_to_bert4rec.py
conda run -n DDBC python DISCO/datasets/MovieLens-20M/len60/convert_disco_to_sasrec.py
...
**关键原则**：
- **DISCO 是训练集切分的唯一来源**。每个 baseline 必须使用 DISCO 的 train/valid/test 切分
- TIGER 和 LETTER 虽然从原始数据处理 Semantic ID，但训练数据必须从 DISCO 的 `train.txt` 反派生（`convert_disco_to_tiger_letter.py`），确保只训练 DISCO 训练集中的 52,857 个 user
- 不这样做会导致数据泄露（TIGER/LETTER 训练时包含了 DISCO 用于 valid/test 的用户）和样本数不一致
- `train_all_ml60.sh` 每次运行时会自动调用 `convert_disco_to_tiger_letter.py`
...
原始数据 (ratings.csv)
    │
    ▼
TIGER train_tiger_movielens20m.sh
    │ → sentence-t5-base.sent_emb (embedding)
    │ → sentence-t5-base_*.sem_ids (semantic id)
    │ → id_mapping.json
    │ → all_item_seqs.json (75511 users)
    │
    ▼
DISCO convert_movielens_to_ddbc.py
    │ → train/valid/test.txt (70/10/20 split, 0-based IDs)
    │ → clhe_sid.npy, clhe_token.json, clhe_weight.npy
    │ → LETTER .index.json
    │
    ▼
Baseline converters (all read from DISCO train.txt)
    │ → DreamRec, DiffuRec, BERT4Rec, SASRec, GRU4Rec
    │ → TIGER all_item_seqs.json (52857 train users only)
    │ → LETTER inter.json (52857 train users only)
```

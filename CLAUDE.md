...
```bash
conda run -n DDBC python DISCO/datasets/Yelp/convert_disco_to_baselines.py
```
...
**For new datasets**: follow the same pattern — generate DISCO's txt files first, then run (or extend) `convert_disco_to_baselines.py` for the new dataset.

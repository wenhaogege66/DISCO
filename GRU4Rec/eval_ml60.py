...
# -*- coding: utf-8 -*-
"""
GRU4Rec – evaluate ML-60 best checkpoint against DDBC evaluation protocol.

Usage:
    conda activate DDBC
    python GRU4Rec/eval_ml60.py --ckpt GRU4Rec/outputs/ml60/start-ml60/best_model.pt
...
from train_movielens60 import ITEM_NUM, evaluate_ddbc, setup_logging


def parse_args():
    p = argparse.ArgumentParser(description='GRU4Rec ML-60 eval — DDBC protocol')
    p.add_argument('--ckpt', required=True, help='Path to best_model.pt')
    p.add_argument('--data_dir',     default=os.path.join(SCRIPT_DIR, 'data/ml60'))
    p.add_argument('--disco_dir',    default=os.path.join(WSPACE_DIR, 'DISCO/datasets/MovieLens-20M/len60'))
    p.add_argument('--dreamrec_dir', default=os.path.join(WSPACE_DIR, 'DreamRec/data/ml60'))
    p.add_argument('--predict_nums',          default='30')
    p.add_argument('--candidate_multipliers', default='19')
    p.add_argument('--predict_mode', default='ar', choices=['single', 'ar'])
...
def main():
    args = parse_args()
    logger = setup_logging(os.path.join(SCRIPT_DIR, 'logs/eval_ml60.log'))
    ...
    logger.info(
        ...
        f'(val_recall@30_x19={val_recall_saved:.4f})'
    )

...
class TrainDataset(data_utils.Dataset):
    def __init__(self, id2seq, max_len, pad_token):
        self.id2seq = id2seq
...
class Data_Train():
    def __init__(self, data_train, args):
        ...
        self.pad_token = args.item_num  # PAD = item_num (0-based IDs, PAD=20033)
        self.split_onebyone()
    ...
    def get_pytorch_dataloaders(self):
        dataset = TrainDataset(self.id_seq, self.max_len, self.pad_token)
...
class ValDataset(data_utils.Dataset):
    def __init__(self, u2seq, u2answer, max_len, pad_token):
        self.u2seq = u2seq
...
class Data_Val():
    def __init__(self, data_train, data_val, args):
        ...
        self.max_len = args.max_len
        self.pad_token = args.item_num

    def get_pytorch_dataloaders(self):
        dataset = ValDataset(self.u2seq, self.u2answer, self.max_len, self.pad_token)
        dataloader = data_utils.DataLoader(dataset, batch_size=self.batch_size, shuffle=False, pin_memory=True)
        return dataloader


class TestDataset(data_utils.Dataset):
    def __init__(self, u2seq, u2_seq_add, u2answer, max_len, pad_token):
        self.u2seq = u2seq
...
class Data_Test():
    ...
    def get_pytorch_dataloaders(self):
        dataset = TestDataset(self.u2seq, self.u2seq_add, self.u2answer, self.max_len, self.pad_token)
        dataloader = data_utils.DataLoader(dataset, batch_size=self.batch_size, shuffle=False, pin_memory=True)
        return dataloader


class CHLSDataset(data_utils.Dataset):
    def __init__(self, data, max_len, pad_token):
        self.data = data
        self.max_len = max_len
        self.pad_token = pad_token
...
class Data_CHLS():
    ...
    def get_pytorch_dataloaders(self):
        dataset = CHLSDataset(self.data, self.max_len, self.pad_token)
        dataloader = data_utils.DataLoader(dataset, batch_size=self.batch_size, shuffle=False, pin_memory=True)
        return dataloader

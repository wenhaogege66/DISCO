...
class Att_Diffuse_model(nn.Module):
    def __init__(self, diffu, args):
        ...
        self.pad_token = args.item_num          # 0-based IDs: items 0..item_num-1, PAD=item_num
        self.item_num = args.item_num + 1       # embedding table size (includes PAD slot)
    ...
    def predict_ddbc(self, sequence, candidate_ids=None):
        """Inference for DDBC evaluation: returns scores over candidates (or full item pool)."""
        item_embeddings = self.item_embeddings(sequence)
        item_embeddings = self.embed_dropout(item_embeddings)
        item_embeddings = self.LayerNorm(item_embeddings)
        mask_seq = (sequence < self.pad_token).float()
        noise_x_t = th.randn_like(item_embeddings[:, -1, :])
        rep_diffu = self.reverse(item_embeddings, noise_x_t, mask_seq)
        if candidate_ids is not None:
            cand_emb = self.item_embeddings(candidate_ids)          # [B, n_cand, H]
            scores = torch.bmm(cand_emb, rep_diffu.unsqueeze(-1)).squeeze(-1)  # [B, n_cand]
        else:
            scores = self.diffu_rep_pre(rep_diffu)                  # [B, item_num]
        return scores
    ...
    def forward(self, sequence, tag, train_flag=True): 
        ...
        mask_seq = (sequence < self.pad_token).float()  # 0-based IDs; PAD=item_num is excluded

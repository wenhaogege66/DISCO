...
class AmazonReviews2023(AbstractDataset):
    ...
    def _process_meta(self, output_path: str):
        ...
        # Try to load from HuggingFace Hub or local cache
        try:
            ...
            # If loading from Hub fails, try to load from manually downloaded parquet files
            self.log(f'[DATASET] Could not load from Hub ({e}), attempting to load from local parquet files...')

            # Find the parquet files in the cache directory
            import glob
            ...
            if not parquet_files:
                raise ValueError(
                    f'Could not find metadata parquet files at {parquet_pattern}. '
                    f'Please download metadata using: '
                    f'huggingface-cli download McAuley-Lab/Amazon-Reviews-2023 '
                    f'--repo-type dataset --include "raw_meta_{self.category}/*" '
                    f'--cache-dir {self.cache_dir}'
                )

            self.log(f'[DATASET] Found {len(parquet_files)} parquet file(s), loading...')
            meta_dataset = load_dataset(
                'parquet',
    ...
    def _download_and_process_raw(self):
        ...
        all_item_seqs_file = os.path.join(processed_data_path, 'all_item_seqs.json')
        metadata_mode = self.config.get('metadata', 'none')
        metadata_file = os.path.join(processed_data_path, f"metadata.{metadata_mode}.json")

        # Check if we can load from cache (all required files exist)
        files_exist = os.path.exists(id_mapping_file) and os.path.exists(all_item_seqs_file)
        metadata_ready = (metadata_mode == 'none') or os.path.exists(metadata_file)

        if files_exist and metadata_ready:
            # Load existing processed data
            self.log(f'[DATASET] Loading processed data from {processed_data_path}')
            with open(id_mapping_file, 'r') as f:
                self.id_mapping = json.load(f)
            with open(all_item_seqs_file, 'r') as f:
            ...
            if os.path.exists(metadata_file):
                ...
                self.item2meta = None
            return
        elif files_exist and not metadata_ready:
            # Interaction data exists but metadata missing - only download metadata
            self.log(f'[DATASET] Loading existing interaction data')
            with open(id_mapping_file, 'r') as f:
                self.id_mapping = json.load(f)
            with open(all_item_seqs_file, 'r') as f:
                self.all_item_seqs = json.load(f)
            # Download and process metadata only

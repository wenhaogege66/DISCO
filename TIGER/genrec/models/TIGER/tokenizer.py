...
class TIGERTokenizer(AbstractTokenizer):
    ...
    def _encode_sent_emb(self, dataset: AbstractDataset, output_path: str):
        ...
        if SentenceTransformer is None:
            raise ImportError(
                'sentence_transformers is required to encode embeddings. '
                'Install it or ensure the .sent_emb cache file exists.'
            )
        ...
        if self.config['sent_emb_pca'] > 0:
            self.log(f'[TOKENIZER] Applying PCA to sentence embeddings before saving (768 -> {self.config["sent_emb_pca"]})...')
            from sklearn.decomposition import PCA
            pca = PCA(n_components=self.config['sent_emb_pca'], whiten=True)
            sent_embs = pca.fit_transform(sent_embs)
            self.log(f'[TOKENIZER] Sentence embeddings shape after PCA: {sent_embs.shape}')
    ...
    def _init_tokenizer(self, dataset: AbstractDataset):
        ...
        if not os.path.exists(sem_ids_path):
            ...
            if os.path.exists(sent_emb_path):
                ...
                # Determine actual embedding dimension based on PCA configuration
                actual_dim = self.config['sent_emb_pca'] if self.config['sent_emb_pca'] > 0 else self.config['sent_emb_dim']
                sent_embs = np.fromfile(sent_emb_path, dtype=np.float32).reshape(-1, actual_dim)
            else:
